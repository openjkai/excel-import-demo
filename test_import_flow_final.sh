...
# Excel Import Application Test Script - Final Version
# Tests the complete import flow with Docker support
...
PHP_CONTAINER="excel_import_app"

# Helper function to run PHP artisan commands in Docker
run_artisan() {
    docker exec "$PHP_CONTAINER" php artisan "$@"
}

# Helper function to run PHP tinker commands
run_tinker() {
    docker exec "$PHP_CONTAINER" php artisan tinker --execute="$1"
}
...
batch_id=$(run_tinker "echo App\Models\ImportBatch::latest()->first()->id ?? 'none';")
...
if echo "$preview_page" | grep -q "Duplicate"; then
    echo "  ✓ Duplicate rows section displayed"
fi
echo ""
...
if echo "$commit_response" | grep -q "Import Status\|Committing"; then
    ...
    echo "⚠ Import commit submitted (checking status)"
fi
echo ""

# Step 10: Wait for the queue job to process
echo "Step 10: Waiting for commit job to process..."
echo "  (Queue worker is running in background container)"
sleep 5
echo "  ✓ Waited for processing"
echo ""
...
if (\$recordCount > 0) {
    ...
    foreach (\$records as \$record) {
        ...
        echo str_pad(substr(\$record->description, 0, 35), 35) . ' | ';
